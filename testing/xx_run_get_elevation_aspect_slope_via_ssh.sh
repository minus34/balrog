...
# run script with a fresh logfile
...
ssh -F ${SSH_CONFIG} ${INSTANCE_ID} "pg_dump -Fc -d geo -t bushfire.bal_factors_gnaf_nsw_slope_only -p 5432 -U ec2-user -f /data/bal_factors_gnaf_nsw_slope_only.dmp --no-owner"
ssh -F ${SSH_CONFIG} ${INSTANCE_ID} "aws s3 cp /data/bal_factors_gnaf_nsw_slope_only.dmp s3://bushfire-rasters/output/"
aws s3 cp s3://bushfire-rasters/output/bal_factors_gnaf_nsw_slope_only.dmp ${HOME}/tmp/bushfire/

# load into local postgres (WARNING: force drops tables first)
/Applications/Postgres.app/Contents/Versions/13/bin/pg_restore -Fc -d geo -p 5432 -U postgres ${HOME}/tmp/bushfire/bal_factors_gnaf_nsw_slope_only.dmp --clean

# display table counts
/Applications/Postgres.app/Contents/Versions/13/bin/psql -d geo -c "select count(*) as address_count from bushfire.bal_factors_gnaf_nsw_slope_only"
